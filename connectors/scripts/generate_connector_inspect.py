...
def check_pb_disabled(pb_json_data):
    try:
        result = {'sanity_test_name': 'Playbooks are Inactive', 'Result': '', 'Status': 'Pass'}
        output = []
...
def check_help_doc(info_json_data):
    try:
        result = {'Test Case': 'Check Online Help Doc', 'Result': '', 'Status': 'Pass'}
...
def convert_json2html(input, connector_name, output_path):
    try:
        process = input
        ...
        print('--->>>> ', output_path + '/output.html')
        html_file = open(output_path + '/output.html', 'w+')
        html_file.write("<html><br><h1>Result for Connector: {name}</h1></br></html>".format(name=connector_name))
        html_file.write(res)
        failed_status = []
        for case in process:
            if case:
...
def run_sanity(connector_info_path, playbook_path, output_path):
    ...
    if os.path.isdir(connector_path):
        if os.path.isdir(connector_path):
            ...
            convert_json2html(final_result, connector_name, output_path)
